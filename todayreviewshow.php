...
function update(_obj){
    var id2 = _obj.responseText;
        var url = "todayreviewshow.php?id="+id2;
        if(id2 > 0){
        	location = url;
        }
        else {alert(
        '您已经完成本次反馈所有题目，点击确定回到反馈主页进行其他操作');
		location = "todayreview.php";
		}
	}
function cross(){
 ajax.query('GET','todayreviewcross.php?id='+$$('id').innerHTML,update);
}
</script>    
...
function last(){
 ...
 ajax4.query('GET','todayreviewlast.php?id='+$$('id').innerHTML,update4);
}
</script>
<script language="javascript" type="text/javascript">
...
<nav id="nav" style="text-align:center">
			...
			<div id="timer_controls" id="last" onclick="location='checkerror.php';" >
				<label >返回</label>
			</div> 
