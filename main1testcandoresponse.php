...
$id1 = $_GET['id'];
      ...
      $sql0 = "select cando from test where name='".$name."'";
	  ...
	  $rs = array_search($id1,$rows2);
	  	if ($rs ==""){
        	$sql = "update test set cando =  concat(cando,'+',$id1) where name='".$name."'";
      		$ok = mysql_query($sql,$conn);
        }
      $sql03 = "select todaycando from test where name='".$name."'";
	  $result03= mysql_query($sql03,$conn);
	  $rows13 = mysql_fetch_array($result03,MYSQL_ASSOC);
	  $string03 = $rows13[todaycando];
	  $rows03 = explode("+",$string03);
	  //print_r($rows2);
	  $rs03 = array_search($id1,$rows03);
	  	if ($rs03 ==""){
        	$sql = "update test set todaycando =  concat(todaycando,'+',$id1) where name='".$name."'";
      		$ok = mysql_query($sql,$conn);
        }
