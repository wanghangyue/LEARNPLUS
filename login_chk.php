...
session_start();
	...
	if(!empty($name) and !empty($pwd)){
		$sql = "select name,count,active from test where name = '".$name."'";
		...
		if($active == ''){
			...
			if($num == 0 or $num == ''){
				$num = $conne->uidRst("update test set count = ".($count+1)." where name = '".$name."'");
				$reback = ($count+1);
			}else{
				if($count != 0){
					$num = $conne->uidRst("update test set count = 0 where name = '".$name."'");
				}
