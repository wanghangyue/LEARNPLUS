...
if (!empty($_GET['name']) && !is_null($_GET['name'])){				//激活注册用户
	$num=$conne->getRowsNum("select * from test where name='".$_GET['name']."' and password = '".$_GET['pwd']."'");
	if ($num>0){
		$upnum=$conne->uidRst("update test set active = 1 where name='".$_GET['name']."' and password = '".$_GET['pwd']."'");
		if($upnum > 0){
