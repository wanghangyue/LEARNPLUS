...
function update(_obj){
    var id2 = _obj.responseText;
        var url = "checkerrorshow.php?id="+id2;
        if(id2 > 0){
        	location = url;
        }
        else {alert(
        '您已经完成本次反馈所有题目，点击确定回到反馈主页进行其他操作');
		location = "checkerror.php";
		}
	}
function cross(){
 ajax.query('GET','checkerrorcross.php?id='+$$('id').innerHTML,update);
}
</script>    
...
function last(){
 ...
 ajax4.query('GET','checkerrorlast.php?id='+$$('id').innerHTML,update4);
}
</script>
<script language="javascript" type="text/javascript">
